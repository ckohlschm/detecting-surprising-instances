...
def dt_regression(pd_data_event_log, descriptive_feature_names, target_feature_name, add_conditions, threshold, max_depth, directory=None):
    ...
    # Create DT Regressor
    clf = tree.DecisionTreeRegressor(max_depth=max_depth, random_state=11, min_samples_leaf=10)
...
def classify(event_log, descriptive_feature_names, target_feature_name, strategy, feature_extraction, parameters, maxDepth=7, num_selected_attributes=0, add_conditions=True, lower_threshold=None, upper_threshold=None):
    if strategy == 'dt':
        return classify_dt(event_log, descriptive_feature_names, target_feature_name, feature_extraction, parameters, maxDepth, num_selected_attributes, add_conditions, lower_threshold, upper_threshold)
    elif strategy == 'nn':
        return classify_nn(event_log, descriptive_feature_names, target_feature_name, parameters)
    elif strategy == 'nb':
        return classify_nb(event_log, descriptive_feature_names, target_feature_name, parameters)
    return []

def detect_surprising_instances(pd_data_event_log, descriptive_feature_names, target_feature_name, strategy, add_conditions, threshold, max_depth, directory=None):
    if strategy == 'dt_regression':
        return dt_regression(pd_data_event_log, descriptive_feature_names, target_feature_name, add_conditions, threshold, max_depth, directory)
    elif strategy == 'nn_regression':
        return nn_regression(pd_data_event_log, descriptive_feature_names, target_feature_name, add_conditions, threshold)
    ...
    elif strategy == 'nn_classification':
        return classify_nn(pd_data_event_log, descriptive_feature_names, target_feature_name)
    elif strategy == 'nb_classification':
        return classify_nb(pd_data_event_log, descriptive_feature_names, target_feature_name)
    else:
...
#pd_data = pd.DataFrame(data, columns=feature_names)
#filtered_attributes = pd_data.filter(items=['event:seriousness_2@Value 1', 'event:seriousness_2@Value 2', 'event:seriousness_2@Value 3', 'event:seriousness_2@Value 4', 'event:support_section@Value 1', 'event:support_section@Value 2', 'event:support_section@Value 3', 'event:support_section@Value 4', 'event:support_section@Value 5', 'event:support_section@Value 6', 'event:support_section@Value 7', 'event:service_level@Value 1', 'event:service_level@Value 2', 'event:service_level@Value 3', 'event:service_level@Value 4', 'event:workgroup@Value 1', 'event:workgroup@Value 2', 'event:workgroup@Value 3', 'event:workgroup@Value 4', 'event:responsible_section@Value 1', 'event:responsible_section@Value 2', 'event:responsible_section@Value 3', 'event:responsible_section@Value 4', 'event:responsible_section@Value 5', 'event:responsible_section@Value 6', 'event:responsible_section@Value 7', 'event:seriousness@Value 1', 'event:service_type@Value 1', 'event:service_type@Value 2', 'event:service_type@Value 3', 'event:service_type@Value 4', 'event:Variant2 index'])
#print('Filtered attributes: ' + str(filtered_attributes))
#feature_names_filtered = filtered_attributes.columns.tolist()
#data_filtered = filtered_attributes.values.tolist()

#target, classes = get_class_representation.get_class_representation_by_str_ev_attr_value_value(log, "concept:name")
#Traces that take longer than 58 days
#target, classes = get_class_representation.get_class_representation_by_trace_duration(log, 58 * 86400)

#event_log, parameters, descriptive_feature_names, target_feature_name = read_event_log('BPI17')

# 4 days 4 * 86400
#lower_threshold = None
# 50 days 50 * 86400
#upper_threshold = 283 * 86400

#maxDepth = 2
#num_selected_attributes = 3
#target_feature_name = ['@@delay']
...
#surprising_instances = classify(event_log, descriptive_feature_names, target_feature_name, 'dt', feature_extraction='other', parameters = parameters, maxDepth=maxDepth, num_selected_attributes=num_selected_attributes, add_conditions=True, lower_threshold=lower_threshold, upper_threshold=upper_threshold)

#surprising_instances = dt_regression(event_log=event_log, descriptive_feature_names=descriptive_feature_names, target_feature_name=target_feature_name, parameters=parameters, maxDepth=maxDepth, num_selected_attributes=num_selected_attributes, add_conditions=True, lower_threshold=None, upper_threshold=None)

#for maxDepth in range(2,6):
#    for days_threshold in np.arange(5, 66, 10):
#        delay_threshold = days_threshold * 86400
#        print('DaysThreshold: ' + str(days_threshold))

#print(surprising_instances[0])
#print("There are " + str(len(surprising_instances)) + " surprising instances")
