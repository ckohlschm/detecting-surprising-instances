...
class Parameters(Enum):
    ...
    ENABLE_COUNT_ACTIVITIES = "enable_count_activities"
...
def count_activity_occurrence(log: EventLog, parameters: Optional[Dict[Union[str, Parameters], Any]] = None) -> Tuple[Any, List[str]]:
    """
    Calculates the occurrence of events in a case
    ...
    if "activity_names_to_count" in parameters:
        activities = parameters["activity_names_to_count"]
    else:
        activities = list(set(y[activity_key] for x in log for y in x))
    ...
    for trace in log:
        num_occurrence_act = {act: 0 for act in activities}
        i = 0
        while i < len(trace)-1:
            act = trace[i][activity_key]
            if act in activities:
                num_occurrence_act[act] = num_occurrence_act[act] + 1
            i = i + 1
        arr = []
        for act in activities:
            arr.append(num_occurrence_act[act])
        data.append(arr)
...
def get_string_trace_attribute_rep(trace: Trace, trace_attribute: str) -> str:
    ...
    if trace_attribute in trace.attributes:
        return "case:" + str(trace_attribute) + "@" + str(trace.attributes[trace_attribute])
    return "case:" + str(trace_attribute) + "@UNDEFINED"
...
def convert_log_to_features(log: EventLog, parameters: Optional[Dict[Union[str, Parameters], Any]] = None) -> Tuple[Any, List[str]]:
    """
    ...
    if type(log) is pd.DataFrame:
        ...
        enable_count_activities = exec_utils.get_param_value(Parameters.ENABLE_COUNT_ACTIVITIES, parameters, enable_all)
        ...
        if enable_count_activities:
            data, features_names = count_activity_occurrence(log, parameters=parameters)
            for i in range(len(datas)):
                datas[i] = datas[i] + data[i]
            features_namess = features_namess + features_names
