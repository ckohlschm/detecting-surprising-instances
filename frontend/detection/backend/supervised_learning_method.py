...
def detect_surprising_instances(request, context):
    ...
    trace_attributes = get_all_trace_attributes_from_log(event_log)
    ...
    threshold = float(request.session['supervised_learning_correlation_threshold'])
    selected_correlation_method = request.session['supervised_learning_correlation_method']
    if selected_correlation_method == 'supervised_learning_correlation_method_assoc_rules':
    ...
    surprising_instances = detect_surprising_instances_algorithm(pd_data_event_log=pd_data_event_log, descriptive_feature_names=feature_list, target_feature_name=target_feature, strategy=model_strategy, add_conditions=True, threshold=model_threshold, max_depth=decision_tree_max_depth)
