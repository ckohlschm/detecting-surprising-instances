...
from pm4py.statistics.attributes.log.get import get_all_trace_attributes_from_log, get_all_event_attributes_from_log, get_attribute_values

from .model_detection import discover_pn_model, discover_bpmn_model
from .data_reader import transform_log_to_features
...
def check_for_post_requests(request, context):
    if request.method == 'POST':
        ...
        if request.session.get('initial_run', True):
            ...
            # Case feature names
            case_attributes = get_all_trace_attributes_from_log(event_log)
            case_attributes = list(case_attributes)
            context['case_attribute_names'] = case_attributes
            print('Case attributes: ' + str(case_attributes))
            request.session['case_attribute_names'] = case_attributes
            # Event feature names
            ...
            all_attribute_names = case_attributes + event_attributes
            print('Attributes: ' + str(all_attribute_names))
            context['all_attribute_names'] = all_attribute_names
            request.session['all_attribute_names'] = all_attribute_names
...
def select_parameters_for_method(request, context):
    # Default definition for parameters
