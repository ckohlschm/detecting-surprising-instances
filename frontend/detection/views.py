...
from .backend.classification_method import apply_supervised_learning
from .backend.clustering_method import apply_clustering
from .backend.root_cause_analysis import apply_root_cause_analysis
    ...
    if request.session.get('log_path', None):
        if request.method == 'POST':
            if 'supervised_learning' in request.POST:
                return HttpResponseRedirect('/method_selection_classification')
            if 'random_walk' in request.POST:
                return HttpResponseRedirect('/method_selection_clustering')
...
def method_selection_classification(request):
    context = {}
    ...
    if request.session.get('log_path', None):
        if request.method == 'POST':
            ...
            if 'submit_parameters_supervised_learning' in request.POST:
                context = process_parameters(request, context)
                return HttpResponseRedirect('/result_classification')
        context = import_log_discover_features_and_model(request, context)
        return render(request, 'detection/method_selection_classification.html', context)
    else:
        return render(request, 'detection/page-404.html', context)

def method_selection_clustering(request):
    context = {}
    ...
    if request.session.get('log_path', None):
        if request.method == 'POST':
            ...
            if 'submit_parameters_random_walk' in request.POST:
                context = process_parameters(request, context)
                return HttpResponseRedirect('/result_clustering')
        context = import_log_discover_features_and_model(request, context)
        return render(request, 'detection/method_selection_clustering.html', context)
    else:
        return render(request, 'detection/page-404.html', context)
...
def result_classification(request):
    ...
    return render(request, 'detection/result_classification.html', context)

def result_clustering(request):
    context = {}
    #context = log_to_features_and_model(request)
    context = apply_clustering(request, context)
    #json_data = detect_surprising_instances_similarity_graph(log_path=request.session['log_path'], pattern_id=None)
    #print(json_data)
    #pd_data = pd.read_json(json_data, orient='split')
    #data_length = len(pd_data.index)
    #print('Surprising instance count: ' + str(data_length))
    #context['surprising_instances'] = pd_data
    return render(request, 'detection/result_clustering.html', context)

def root_cause_analysis_classification(request):
     context = {}
     context['selected_leaf_id'] = request.session['selected_leaf_id']
     context = apply_root_cause_analysis(request, context)
     return render(request, 'detection/root_cause_analysis_classification.html', context)